...
import { Chain, logger, redisConnection } from '.';
...
export const createPool = async (chain: Chain, seeds: PoolSeeds): Promise<number | null> => {
  ...
  if (!Number.isNaN(poolId)) {
    if (poolId == 0) {
      ...
      await new Queue('pool-archiver', { connection: redisConnection }).add(
        'archive-pool',
        { poolId, chain },
        {
      ...
      await new Queue('pool-completer', { connection: redisConnection }).add(
        'complete-pool',
        { poolId, chain },
        {
