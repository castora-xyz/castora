...
import { readGettersContract } from './contract.js';
...
export const fetchPool = async (chain: Chain, poolId: any): Promise<Pool> => {
  ...
  const noOfPools = Number(((await readGettersContract(chain, 'allStats')) as any).noOfPools);
  ...
  const raw = await readGettersContract(chain, 'pool', [BigInt(poolId)]);
  if (!raw) {
