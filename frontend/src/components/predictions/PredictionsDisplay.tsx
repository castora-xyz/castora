...
export const PredictionsDisplay = ({ pool: { noOfPredictions, seeds, completionTime }, pool }: { pool: Pool }) => {
  ...
  return (
    <div className="border border-border-default dark:border-surface-subtle p-6 rounded-[24px] w-full mb-8">
      <h3 className="font-medium text-xl text-text-subtitle mb-6">Pool Activities</h3>
      ...
      {isFetching ? (
        [1, 2, 3, 4, 5].map((i) => <Breathing key={i} height={64} className="mb-3 rounded-2xl w-full" />)
      ) : hasError ? (
      ...
      ) : predictions.filter(({ predicter }) => predicter != address).length == 0 ? (
        <div className="flex flex-col justify-center items-center grow text-center py-20">
          ...
          <p className="text-lg sm:text-2xl">
            No {noOfPredictions > 0 ? 'Other' : ''} Pool Activity {now >= seeds.windowCloseTime ? '' : 'Yet'}
          </p>
        </div>
      ) : (
        predictions
          ...
          .map((prediction) => {
            const { id, isAWinner, predicter, price, time, claimWinningsTime } = prediction;
            return (
              <div
                key={id}
              ...
              >
                ...
                >
                  {ms((now - time) * 1000)} {now < seeds.snapshotTime ? 'ago' : ''}
                </p>
